...
#include<QDebug>
#include<QMessageBox>
GetBackWindow::GetBackWindow(QWidget *parent, MainWindow* obj) :
        QWidget(parent) {
    this->setWindowTitle("还原环境变量");
    mw = obj;
    /*
     MainWindow* mw;
    ...
    lbDescribe = new QLabel("选择的备份文件:");
    hblRow1->addWidget(lbDescribe);
    pbChoseFile = new QPushButton("选择导入的备份文件", this);
...
//    lbFileInfo->setAlignment(Qt::AlignTop);
    ...
    pbUseBack = new QPushButton("确认还原", this);
...
void GetBackWindow::closeEvent(QCloseEvent *event) {
    mw->setDisabled(false);
    qDebug() << event;
}

void GetBackWindow::slotChoseFile() {
    strBackPath = QFileDialog::getOpenFileName(this, "选择已存在的备份文件", "./back",
            "Files(*.back)");
...
void GetBackWindow::SlotUseBack() {
    if (mw->isAdimin == true) {
